...
```bash
dbt run --select tag:hourly --project-dir hands-on/dbt/dbt_learn
```

### 週次バッチ（毎週日曜 — full-refresh 付き）

```bash
dbt run --select tag:weekly_heavy --full-refresh --project-dir hands-on/dbt/dbt_learn
```
...
```bash
# まず影響範囲を確認
dbt list --select stg_customers+ --project-dir hands-on/dbt/dbt_learn
