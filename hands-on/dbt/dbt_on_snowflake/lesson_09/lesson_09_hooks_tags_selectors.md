...
> **やること**: `hands-on/dbt/dbt_learn/models/marts/dim_customers.sql` の config に post-hook を追加する。
...
```bash
dbt compile --select dim_customers --project-dir hands-on/dbt/dbt_learn
```
...
> **やること**: `hands-on/dbt/dbt_learn/models/marts/agg_daily_orders.sql` に pre-hook を追加する。
...
```bash
dbt run --select agg_daily_orders --project-dir hands-on/dbt/dbt_learn
```
...
> **やること**: `hands-on/dbt/dbt_learn/dbt_project.yml` に以下を追加する。
...
さらに、個別モデルにもタグを付与できる。  
`hands-on/dbt/dbt_learn/models/marts/agg_daily_orders.sql` の config：
...
```bash
# "daily" タグが付いたモデルだけ実行
dbt run --select tag:daily --project-dir hands-on/dbt/dbt_learn

# "reporting" タグのモデルだけテスト
dbt test --select tag:reporting --project-dir hands-on/dbt/dbt_learn

# "staging" タグを除外して実行
dbt run --exclude tag:staging --project-dir hands-on/dbt/dbt_learn
```
...
dbt run --select +stg_customers+ --project-dir hands-on/dbt/dbt_learn
```
...
dbt run --select models/marts/ --project-dir hands-on/dbt/dbt_learn
```
...
```bash
# marts かつ daily タグ
dbt run --select tag:daily,path:models/marts --project-dir hands-on/dbt/dbt_learn
```
...
dbt list --select stg_orders+ --project-dir hands-on/dbt/dbt_learn
```
...
dbt list --select stg_orders+ --resource-type model --project-dir hands-on/dbt/dbt_learn
```
...
```bash
dbt run --project-dir hands-on/dbt/dbt_learn
```
