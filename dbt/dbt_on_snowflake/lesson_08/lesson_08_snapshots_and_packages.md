...
> **やること**: `dbt/dbt_learn/snapshots/snap_customers.sql` を作成する。
...
```bash
dbt snapshot --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/snapshots/snap_customers_sim.sql` を作成する。
...
```bash
# 初回実行
dbt snapshot --select snap_customers_sim --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/packages.yml` を作成する。
...
> **やること**: `dbt/dbt_learn/models/marts/fct_line_items_v2.sql` を作成する。
...
```bash
# compile で生成 SQL を確認
dbt compile --select fct_line_items_v2 --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/models/marts/marts.yml` に以下のテストを追加する。
...
```bash
# モデル作成 + テスト実行
dbt build --select fct_line_items_v2 --project-dir dbt/dbt_learn
```
...
```bash
dbt test --select fct_line_items_v2 --project-dir dbt/dbt_learn
```
