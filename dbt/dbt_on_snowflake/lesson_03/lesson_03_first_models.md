...
> **やること**: `dbt/dbt_learn/models/staging/stg_orders.sql` を開き確認する。
...
```bash
dbt compile --select stg_orders --project-dir dbt/dbt_learn
```
...
```bash
dbt run --select staging --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/models/marts/fct_orders.sql` の内容を確認する。
...
```bash
dbt compile --select fct_orders --project-dir dbt/dbt_learn
```
