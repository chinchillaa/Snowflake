...
`dbt/dbt_learn/models/marts/recent_orders.sql` を新規作成：
...
```bash
dbt compile --select recent_orders --project-dir dbt/dbt_learn
```
...
```bash
dbt compile --select recent_orders --vars '{"start_date": "1995-06-01"}' --project-dir dbt/dbt_learn
```
...
```bash
dbt run --select recent_orders --project-dir dbt/dbt_learn
```
...
> **やること**: 新規ファイル `dbt/dbt_learn/models/marts/order_status_pivot.sql` を作成。
...
```bash
dbt compile --select order_status_pivot --project-dir dbt/dbt_learn
```
...
```bash
dbt run --select order_status_pivot --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/macros/limit_in_dev.sql` を確認する。
...
> **やること**: `dbt/dbt_learn/macros/clean_string.sql` を新規作成する。
...
```bash
dbt compile --select stg_customers --project-dir dbt/dbt_learn
```
...
> **やること**: `dbt/dbt_learn/packages.yml` を新規作成する。
...
```bash
dbt deps --project-dir dbt/dbt_learn
```
...
```bash
dbt compile --project-dir dbt/dbt_learn
```
