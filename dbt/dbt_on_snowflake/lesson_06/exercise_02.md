...
1. ファイル: `dbt/dbt_learn/models/marts/customer_return_summary.sql` を新規作成
...
```bash
# 1. compile で展開を確認
dbt compile --select customer_return_summary --project-dir dbt/dbt_learn
...
# 3. 実行
dbt run --select customer_return_summary --project-dir dbt/dbt_learn
```
