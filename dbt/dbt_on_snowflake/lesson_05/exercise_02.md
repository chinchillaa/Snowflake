...
`dbt/dbt_learn/models/staging/staging.yml` に stg_line_items のテスト定義がすでに一部あるが、以下を追加：
...
`dbt/dbt_learn/tests/assert_line_item_quantity_positive.sql` を作成：
- `quantity` が 0 より大きいことを検証

`dbt/dbt_learn/tests/assert_discount_in_range.sql` を作成：
...
```bash
# 全テストを実行
dbt test --select stg_line_items --project-dir dbt/dbt_learn
```
